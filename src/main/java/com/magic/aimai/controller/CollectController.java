...
public class CollectController extends BaseController {
    ...
    public ViewData queryCollect(Integer type,Integer pageNO,Integer pageSize){
        try {
            ...
            return buildSuccessJson(StatusConstant.SUCCESS_CODE,"获取成功",
                    collectService.queryCollectByTypeOfWeb(type,user.getId(),pageNO,pageSize));
    ...
    public ViewData delete(Integer type, Integer targetId){
        try {
            User user =  LoginHelper.getCurrentUserOfAPI();
            if (null == user){
            ...
            if (type < 0 || type > 2) {
                return buildFailureJson(StatusConstant.ARGUMENTS_EXCEPTION,"未知的类型");
            }
            collectService.delete(user.getId(),type,targetId);
        } catch (InterfaceCommonException e) {
            return buildFailureJson(e.getErrorCode(),e.getMessage());
        } catch (Exception e) {
            logger.error(e.getMessage(),e);
            return buildFailureJson(StatusConstant.Fail_CODE,"取消收藏失败");
        }
        return buildSuccessCodeJson(StatusConstant.SUCCESS_CODE,"取消收藏成功");
    }
