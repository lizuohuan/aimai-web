...
public class SystemInfoController extends BaseController {
     ...
     * @return
     */
    @RequestMapping(value = "/geySystemInfo",method = RequestMethod.POST)
    ...
    public ViewData geySystemInfo(Integer pageNO,Integer pageSize){
        try {
            User user = LoginHelper.getCurrentUserOfAPI();
            return buildSuccessJson(StatusConstant.SUCCESS_CODE,"获取成功",
                    systemInfoService.queryInfoOfWeb(user.getId(),null == pageNO ? null :
                            (pageNO - 1),pageSize));
