...
public class UserController extends BaseController {
    ...
    @ApiOperation(value = " 登录时候获取验证码接口")
     ...
     * @return
     */
    @RequestMapping("/getFreeUser")
    public ViewData getFreeUser(Integer pageNO,Integer pageSize){
        try {
            User company = LoginHelper.getCurrentUserOfAPI();
            if(RoleEnum.BUSINESS_USER.ordinal() != company.getRoleId()){
                return buildFailureJson(StatusConstant.NOT_AGREE,"没有权限");
            }
            return buildSuccessViewData(StatusConstant.SUCCESS_CODE,"操作成功",
                    userService.queryFreeUser(company.getId(),pageNO,pageSize));
        } catch (InterfaceCommonException e) {
            return buildFailureJson(e.getErrorCode(),e.getMessage());
        } catch (Exception e) {
            logger.error(e.getMessage(),e);
    ...
    public ViewData importExcel(String url){
        ...
        try {
            ...
            Map<String, Object> data = userService.importExcelUser(url, company.getId());
            return buildSuccessViewData(StatusConstant.SUCCESS_CODE,"操作成功",
                    data);
        } catch (InterfaceCommonException e) {
            return buildFailureJson(e.getErrorCode(),e.getMessage());
    ...
    public void exportExcelUser(String searchParam,Integer companyId,HttpServletResponse response,String userIds){
        try {
            if(null == companyId){
                response.getWriter().print(JSONObject.fromObject(buildFailureJson(StatusConstant.FIELD_NOT_NULL,"字段不能为空")));
            }
            List<Integer> userIdList = null;
            if(!CommonUtil.isEmpty(userIds)){
                userIdList = Arrays.asList(ClassConvert.strToIntegerGather(userIds.split(",")));
            }
            List<User> users = userService.queryUserByParent(null, searchParam,userIdList);
            ExcelUtil.exportExcelUser(response,"detail",users);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void exportExcel(Long yearTimeStamp, Integer stageId, Integer cityId, Integer levelType,
                                String searchParams, HttpServletResponse response){
        try {
            stageId = stageId == 0 ? null : stageId;
            searchParams = CommonUtil.isEmpty(searchParams) ? null : searchParams;
            Date year = null == yearTimeStamp ? null : new Date(yearTimeStamp);
            List<WebStatistics> webStatisticses = userService.exportExcel(year, stageId, cityId,
                    levelType, searchParams);
            ExcelUtil.drawExcel(response,"detail",webStatisticses);
        } catch (Exception e) {
            logger.error(e.getMessage(),e);
        }
     ...
     * @return
     */
    @RequestMapping(value = "/webStatistics",method = RequestMethod.POST)
    public ViewData webStatistics(Long yearTimeStamp,Integer stageId,Integer cityId,Integer levelType,
                                  String searchParams){
        try {
            User user = LoginHelper.getCurrentUserOfAPI();
            ...
            if(null == cityId && RoleEnum.GOVERNMENT_USER.ordinal() == user.getRoleId()){
                cityId = user.getCityId();
                levelType = 3;
            }
            Set<Integer> allCityIds = new HashSet<Integer>();
            if(null == cityId && RoleEnum.BUSINESS_USER.ordinal() == user.getRoleId()){
                List<City> cities = cityService.queryCityByUser(user.getId()); // 查询这些城市下的企业的游离状态员工
                if(null != cities && cities.size() > 0){
                    ...
                    if(provinceIds.size() > 0){
                        List<City> cities1 = cityService.queryCitys(provinceIds); // 市级
                    ...
                    if(cityIds.size() > 0){
                        List<City> cities1 = cityService.queryCitys(cityIds); // 区县
                        if(null != cities1 && cities1.size() > 0){
                            for (City city : cities1) {
                                countyIds.add(city.getId());
                            }
            ...
            return buildSuccessViewData(StatusConstant.SUCCESS_CODE,"获取成功",
                    userService.webStatistics(null == yearTimeStamp ? null : new Date(yearTimeStamp),
                            stageId,cityId,levelType,searchParams,allCityIds));
        } catch (InterfaceCommonException e) {
            return buildFailureJson(e.getErrorCode(),e.getMessage());
        } catch (Exception e) {
            logger.error(e.getMessage(),e);
    ...
    public ViewData queryCompanyList(String searchParams,Integer cityId,Integer levelType,Integer pageNO,Integer pageSize){
        ...
        try {
            ...
            if(null == cityId && RoleEnum.GOVERNMENT_USER.ordinal() != user.getRoleId()){
                cityId = user.getCityId();
                levelType = 3;
            }else if(null == cityId && RoleEnum.BUSINESS_USER.ordinal() != user.getRoleId()){
                List<City> cities = cityService.queryCityByUser(user.getId()); // 查询这些城市下的企业的游离状态员工
                if(null != cities && cities.size() > 0){
                    List<Integer> provinceIds = new ArrayList<Integer>();
                    List<Integer> cityIds = new ArrayList<Integer>();
            ...
            return buildSuccessViewData(StatusConstant.SUCCESS_CODE,"获取成功",
                    userService.queryUserForGovernment(searchParams,cityId,levelType,pageNO,pageSize,allCityIds));
        } catch (InterfaceCommonException e) {
            return buildFailureJson(e.getErrorCode(),e.getMessage());
        } catch (Exception e) {
            logger.error(e.getMessage(),e);
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "isAllocation",value = "是否是课程分配时获取的用户列表 0:否  1：是",dataType = "path",paramType = "integer"),
            @ApiImplicitParam(name = "pageNO",value = "分页参数", required = true ,dataType = "path",paramType = "Integer.class"),
            @ApiImplicitParam(name = "pageSize",value = "分页参数", required = true ,dataType = "path",paramType = "Integer.class"),
    ...
    public ViewData queryUserByCompany(String searchParam,Integer orderId,Integer isAllocation,
        ...
        try {
            ...
            return buildSuccessViewData(StatusConstant.SUCCESS_CODE,"获取成功",
                    userService.queryUserByCompanyOfWeb(companyUser.getId(),searchParam,
    ...
    @RequestMapping(value = "/getAllocationedUser",method = RequestMethod.POST)
    @ApiOperation(value = "获取已分配课程的用户")
    @ApiImplicitParam(name = "curriculumId",value = "课程ID",required = true)
    public ViewData getAllocationedUser(Integer curriculumId){
        ...
        try {
            User user = LoginHelper.getCurrentUserOfAPI();
            return buildSuccessViewData(StatusConstant.SUCCESS_CODE,"获取成功",
                    userService.getUserByCurriculum2(curriculumId,user.getId()));
        }  catch (InterfaceCommonException e) {
            return buildFailureJson(e.getErrorCode(),e.getMessage());
        } catch (Exception e) {
            logger.error(e.getMessage(),e);
    ...
    public ViewData addColleague(Integer userId,String departmentName,String jobTitle){
        ...
        try {
            ...
            if(RoleEnum.COMPANY_USER.ordinal() != user.getRoleId() && RoleEnum.BUSINESS_USER.ordinal() != user.getRoleId()){
                return buildFailureJson(StatusConstant.NOT_AGREE,"没有权限");
            }
   ...
   @ApiImplicitParam(name = "phone",value = "手机号",dataType = "String.class")})
    ...
    public ViewData updateUser(String showName,String avatar,Integer cityId,Integer definition,
        ...
        try {
            ...
            // 更新缓存
            LoginHelper.del(user.getToken());
            LoginHelper.add(user.getToken(),user);
        } catch (InterfaceCommonException e) {
            return buildFailureJson(e.getErrorCode(),e.getMessage());
        }catch (Exception e) {
            logger.error(e.getMessage(),e);
    ...
    public ViewData login(String phone,String password,Integer deviceType,String deviceToken,
        ...
        try {
            ...
            if(RoleEnum.COMPANY_USER.ordinal() != user.getRoleId() && RoleEnum.USER.ordinal() != user.getRoleId()
                    && RoleEnum.GOVERNMENT_USER.ordinal() != user.getRoleId()  && RoleEnum.BUSINESS_USER.ordinal() != user.getRoleId()){
                return buildFailureJson(StatusConstant.NOT_AGREE,"没有权限");
            }
            ...
            if (null != user.getCityId()) {
                City city = cityService.getThreeId(user.getCityId());
                JSONArray jsonArray = new JSONArray();
                jsonArray.add(city.getProvinceId());
                jsonArray.add(city.getTownId());
                jsonArray.add(city.getId());
            ...
            // 日志处理
            // 记录日志
            int i = loginLogService.loginLog(request,user,StatusConstant.LOG_LOGIN_PC );
            if(i == 1){
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "phone",value = "手机号",required = true,paramType = "string",dataType = "String.class"),
            @ApiImplicitParam(name = "code",value = "验证码",required = true,paramType = "string",dataType = "String.class"),
            @ApiImplicitParam(name = "deviceType",value = "设备类型 android:0,ios:1,微信端为空",paramType = "int",dataType = "Integer.class"),
            @ApiImplicitParam(name = "deviceToken",value = "设备注册ID",paramType = "string",dataType = "String.class"),
            @ApiImplicitParam(name = "isWechat",value = "除微信端传 1，其他传 0",paramType = "int",dataType = "Integer.class")
    })
    public ViewData loginCode(String phone,String code,Integer deviceType,String deviceToken,
                          Integer isWechat){

        if(CommonUtil.isEmpty(phone,code,isWechat)){
            return buildFailureJson(StatusConstant.FIELD_NOT_NULL,"字段不能为空");
        }
        try {
            Object o = LoginHelper.get("sms_" + phone);
            if(null == o || !code.equals(o.toString())){
                 return buildFailureJson(StatusConstant.Fail_CODE,"验证码错误");
            }
            User user = userService.queryBaseUserByPhone(phone);
            // 后台用户不能登录APP
            if(RoleEnum.COMPANY_USER.ordinal() != user.getRoleId() && RoleEnum.USER.ordinal() != user.getRoleId()
     ...
     * @param id
     */
    @RequestMapping(value = "/removeParentId",method = RequestMethod.POST)
    @ApiOperation(value = "与公司脱离关系 与上级用户解除关联",response = ViewData.class)
    @ApiImplicitParam(name = "id",value = "用户id",paramType = "Integer",dataType = "Integer.class")
    public ViewData removeParentId(Integer id){
        try {
            if(CommonUtil.isEmpty(id)){
                return buildFailureJson(StatusConstant.FIELD_NOT_NULL,"字段不能为空");
            }
            userService.removeParentId(id);
